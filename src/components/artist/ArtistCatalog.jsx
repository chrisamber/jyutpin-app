...
function AlbumThumb({ title, artist }) {
  ...
  if (!src) {
    return (
      <div className="w-9 h-9 rounded-md bg-bg-elevated flex items-center justify-center flex-shrink-0">
  ...
  return (
    <div className="relative mb-8 rounded-2xl overflow-hidden">
      ...
      <div className="relative flex items-end gap-5 px-6 pt-10 pb-6">
        ...
        <div>
          <h1 className="text-3xl font-light tracking-widest text-text-primary cjk leading-tight">
            {artist}
          </h1>
          {artistEn && artistEn !== artist && (
            <p className="text-sm text-text-secondary font-mono mt-0.5">{artistEn}</p>
          )}
  ...
  return (
    <div>
      <button
        ...
        className="mb-5 text-[11px] font-mono text-text-secondary hover:text-accent transition-colors"
      >
        ← back
      </button>
      ...
      <div className="space-y-0.5">
        {entry.songs.map((song, i) => (
          <button
            ...
            className="w-full flex items-center gap-3 px-3 py-2.5 rounded-xl text-left hover:bg-bg-elevated transition-colors group disabled:opacity-40"
          >
            <AlbumThumb title={song.title} artist={entry.artist} />

            <div className="flex-1 min-w-0">
              <div className="text-sm cjk text-text-primary group-hover:text-accent transition-colors truncate">
                {song.title}
              </div>
              {song.album && (
                <div className="text-[11px] text-text-secondary font-mono truncate mt-0.5">
                  {song.album}
                </div>
              )}
            </div>
            ...
            <div className="flex items-center gap-3 flex-shrink-0">
              {song.year && (
                <span className="text-[11px] font-mono text-text-muted">{song.year}</span>
              )}
              {loading === song.title ? (
                <span className="text-xs text-accent/60 font-mono">…</span>
              ) : (
                <span className="text-text-muted group-hover:text-accent/50 transition-colors text-xs opacity-0 group-hover:opacity-100">▶</span>
              )}
            </div>
          </button>
        ))}
